...
export default function Account() {
  ...
  const [photoUrl, setPhotoUrl] = useState(localStorage.getItem('profile-photo') || '')
  ...
  return <main className="min-h-screen bg-gray-50 px-4 py-12"><div className="mx-auto max-w-2xl space-y-6">
    ...
    <div className="rounded-2xl bg-white p-6 shadow-sm space-y-4">
      <h2 className="text-lg font-bold text-[#0D2137]">Photo de profil</h2>
      <div className="flex items-center gap-4">
        {photoUrl ? (
          <img src={photoUrl} alt="Photo de profil" className="w-16 h-16 rounded-full object-cover border" onError={e => (e.currentTarget.style.display = 'none')} />
        ) : (
          <div className="w-16 h-16 rounded-full bg-[#C9A84C] flex items-center justify-center text-white text-xl font-bold">
